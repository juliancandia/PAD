...
infile = file.path(PROJECT_DIR,"RESULTS","FIG1","Supplementary_Data_2.xlsx")
...
outfile = file.path(PROJECT_DIR,"RESULTS","FIG1","Pathways_RAW.txt")
output = cbind(1:n_pwy,pwy,ref_annot,pwy_label,res[,2:3])
write(t(output),ncol=ncol(output),file=outfile,sep="\t")
infile = file.path(PROJECT_DIR,"RESULTS","FIG1","Pathways_EDITED.txt")
...
outfile = file.path(PROJECT_DIR,"RESULTS","FIG1","GSEA_barplot.pdf")
